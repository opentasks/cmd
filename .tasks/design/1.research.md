...
type Task struct {
    ...
    ID          string         // Semantic ID (e.g., "s-42", "e-5")
...
// Prefix to type mapping (inverse)
...
type Relationship struct {
    ...
    TaskID string // Target task ID (e.g., "s-42", "e-5")
}
...
Task IDs follow a predictable pattern: `{type-prefix}-{sequence}{collision-suffix}`

```go
// Examples:
// e-1      (first epic)
// s-42     (42nd story)
// p-3a     (collision on 3rd plan, first variant)
// d-1b     (collision on 1st decision, second variant)

// ID validation regex pattern
const IDPattern = `^[erpsdt]-\d+[a-z]*$`

// ID format components:
// Type prefix: [erpsdt] (one char)
// Dash separator: -
// Sequence number: \d+ (one or more digits, no leading zeros)
// Collision suffix: [a-z]* (optional, a-z for collisions)
```
